...
- ESP-to-Server communication uses TLS 1.2+ with certificate pinning to prevent man-in-the-middle attacks
- Observability requires structured logging with distributed tracing for end-to-end request tracking

## Clarifications

### Session 2026-05-17
- Q: What specific security measures should be implemented for ESP-to-Server communication beyond the device token authentication? → A: TLS 1.2+ with certificate pinning
- Q: What level of observability (logging, metrics, tracing) is required for debugging ESP-Server synchronization issues? → A: Structured logging with distributed tracing
